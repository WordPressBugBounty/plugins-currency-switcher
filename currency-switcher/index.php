...
final class WPCS {
    ...
    public function print_plugin_options() {
        ...
        $this->render_html_e(WPCS_PATH . 'views/plugin_options.php', $args);
    }
...
//we need it just only for ajax update
    ...
    public function plugin_action_links($links) {
        ...
        if ($this->show_notes) {
            $buttons[] = '<a target="_blank" style="color: red;" href="https://codecanyon.pluginus.net/item/wordpress-currency-switcher/17450674">' . esc_html__('Go Pro!', 'currency-switcher') . '</a>';
        }
    ...
    public function wp_head() {
        ...
        <script type="text/javascript">
            ...
            var wpcs_drop_down_view = "<?php echo esc_html($this->get_drop_down_view()); ?>";
        ...
        <?php endif; ?>
            ...
            var wpcs_ajaxurl = "<?php echo esc_attr(admin_url('admin-ajax.php')); ?>";
            var wpcs_lang_loading = "<?php esc_html_e('loading', 'currency-switcher') ?>";
...
//***
    ...
    public function save_etalon() {
        ...
        if (!isset($_REQUEST['options_nonce']) || !wp_verify_nonce($_REQUEST['options_nonce'], 'wpcs_save')) {
            return "";
        }
        $currencies = $this->get_currencies();
    ...
    public function wpcs_convert_currency() {
        wp_die(esc_html($this->convert_currency($_REQUEST['from'], $_REQUEST['to'], $_REQUEST['amount'], $_REQUEST['precision'])));
    }
        ...
        add_action('admin_notices', function () use ($slug) {
            ...
            <div class="notice notice-info" id="pn_<?php echo esc_attr($slug) ?>_ask_favour" style="position: relative;">
                ...
                <div id="pn_<?php echo esc_attr($slug) ?>_review_yes" style="display: none;">
                    <p><?php printf(esc_html__('That\'s awesome! Could you please do us a BIG favor and give it a 5-star rating on %s to help us spread the word and boost our motivation?', 'currency-switcher'), esc_html($on)) ?></p>
                    <p style="font-weight: bold;">~ PluginUs.NET developers team</p>
                    <p>
                        <a href="<?php echo esc_attr($link) ?>" style="display: inline-block; margin-right: 10px;" onclick="pn_<?php echo esc_attr($slug) ?>_dismiss_review(2)" target="_blank"><?php esc_html_e('Okay, you deserve it', 'currency-switcher'); ?></a>
                        <a href="javascript: pn_<?php echo esc_attr($slug) ?>_dismiss_review(1); void(0);" style="display: inline-block; margin-right: 10px;"><?php esc_html_e('Nope, maybe later', 'currency-switcher'); ?></a>
                        <a href="javascript: pn_<?php echo esc_attr($slug) ?>_dismiss_review(2); void(0);"><?php esc_html_e('I already did', 'currency-switcher'); ?></a>
                    </p>
                </div>

                <div id="pn_<?php echo esc_attr($slug) ?>_review_no" style="display: none;">
                    <p><?php esc_html_e('We are sorry to hear you aren\'t enjoying WPCS. We would love a chance to improve it. Could you take a minute and let us know what we can do better?', 'currency-switcher'); ?></p>
                    <p>
                        <a href="https://pluginus.net/contact-us/" onclick="pn_<?php echo esc_attr($slug) ?>_dismiss_review(2)" target="_blank"><?php esc_html_e('Give Feedback', 'currency-switcher'); ?></a>&nbsp;
                        |&nbsp;<a href="javascript: pn_<?php echo esc_attr($slug) ?>_dismiss_review(2); void(0);"><?php esc_html_e('No thanks', 'currency-switcher'); ?></a>
                    </p>
                </div>
